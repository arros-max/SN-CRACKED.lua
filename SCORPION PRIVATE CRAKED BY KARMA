...
-- VENTANA PRINCIPAL
local window = library:AddWindow("SCORPION PRIVATE CRAKED BY KARMA  | Bienvenido " .. displayName, {
...
AutoFarm:AddSwitch("Fuerza OP (HYPER)", function(state)
...
AutoFarm:AddSwitch("Black mode (Full Black)", function(State)
    local lighting = game:GetService("Lighting")
    if State then
...
AutoFarm:AddSwitch("Anti AFK", function(state)
    if state then
        ...
        uselesslabelone.Text = "Anti Afk By SN"
...
AutoFarm:AddButton("TP Jungle Squat", function()
...
local RebirthFolder = AutoFarm:AddFolder("Rebirth PACK")
...
Credits:AddLabel("SCORPION HUB BY SN CRAKED BY KARMA")
Credits:AddLabel("Script Hecho por: SN CRAKED BY KARMA")
Credits:AddLabel("CLAN FZKONTOP 🔥")
